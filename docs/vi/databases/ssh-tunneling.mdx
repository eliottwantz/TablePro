...
TablePro hỗ trợ ba phương thức xác thực SSH:
...
<Tabs>
  ...
  <Tab title="SSH Agent">
    Dùng agent SSH sẵn có (ví dụ: 1Password hoặc `ssh-agent` của hệ thống):

    | Field | Mô tả |
    |-------|-------------|
    | **Agent Socket** | Đường dẫn tới socket của agent. Để trống để dùng `SSH_AUTH_SOCK`. |

    <Tip>
    Nhấp **1Password Default** để điền `~/Library/Group Containers/2BUA8C4S2C.com.1password/t/agent.sock`.
    </Tip>
  </Tab>
</Tabs>

{/* Screenshot: Phương thức xác thực SSH */}
<Frame caption="Xác thực SSH: các tùy chọn">
  <img
    className="block dark:hidden"
    src="/images/ssh-auth-methods.png"
    alt="Các tùy chọn xác thực SSH"
  />
  <img
    className="hidden dark:block"
...
Để biết đầy đủ thông số URL bao gồm tất cả các tham số truy vấn, xem [Tham chiếu URL kết nối](/databases/connection-urls#ssh-tunnel-format).
