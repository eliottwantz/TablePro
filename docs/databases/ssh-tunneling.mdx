...
<Tabs>
  ...
  <Tab title="SSH Agent">
    Use an existing SSH agent (e.g., 1Password or system `ssh-agent`):

    | Field | Description |
    |-------|-------------|
    | **Agent Socket** | Path to the agent socket. Leave blank to use `SSH_AUTH_SOCK`. |

    <Tip>
    Click **1Password Default** to fill `~/Library/Group Containers/2BUA8C4S2C.com.1password/t/agent.sock`.
    </Tip>
  </Tab>
</Tabs>

{/* Screenshot: SSH authentication methods */}
<Frame caption="SSH authentication options">
  <img
    className="block dark:hidden"
    src="/images/ssh-auth-methods.png"
    alt="SSH authentication options"
  />
  <img
    className="hidden dark:block"
...
For the full URL specification including all query parameters, see [Connection URL Reference](/databases/connection-urls#ssh-tunnel-format).
